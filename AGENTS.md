...
CrabCrush（小螃蟹）是一个面向中国用户的本地优先、风险可控的个人 AI 执行助手，
支持本地 Web 控制面以及企业微信/钉钉/飞书等入口，深度集成国产大模型，提供更符合中国人使用习惯的智能执行体验。
...
3. **入口多样，但本地控制面优先**：聊天渠道适合触发与通知，执行类任务优先在本地 Web 控制面完成
4. **模型无关（Model-Agnostic）**：支持国产和国际大模型，用户自由选择
5. **安全可控（Safety-First）**：高风险操作必须确认、可审计、可中断
6. **插件化扩展（Plugin-First）**：通过 Skills 系统扩展能力，但建立在稳定执行原语之上
...
风险可控的电脑/知识环境操作、本地控制面、中文语境理解、国内渠道接入、合规与安全、网络适配。
...
- **Phase 2a（基础版已交付）**：Token 认证、SQLite 对话持久化 + 滑动窗口、Function Calling + Owner 权限、内置工具（time/browser/search/file）、工作区人格化（DEC-032）
- **执行安全（基础版已交付）**：`confirmRequired`（工具级）+ `permission_request`（请求级）+ `once/session/persistent` 授权作用域 + 执行预览 + 拒绝后降级 + 审计回放 已在运行时生效，WebChat/钉钉均支持确认
- **Web 控制面（基础版已交付）**：当前任务、审计回放、授权中心（会话授权 + 长期授权）、工作区设置 已并入 WebChat 右侧工作台

### 下一步（Phase 2a 续，按 DEC-035 收束）
- [ ] 收口权限模型：资源粒度、权限中心展示/撤销、连接数限制与入口级限流配置化
- [ ] Web 控制面：把“确认弹窗”继续升级成更强的任务确认与执行摘要体验
- [ ] 高价值工具闭环：浏览器会话复用 + 截图/填表、数据库查询
- [ ] 受限执行路线先定边界：预定义修复动作 → 受限 `run_command` → 真正代码沙箱，避免直接做成“大而乱”的 exec
- [ ] 其后再做 Skills 框架与飞书渠道
