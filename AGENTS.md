...
- **安全确认（已实现）**：`confirmRequired`（工具级）与 `permission_request`（请求级）已在运行时生效，WebChat/钉钉均支持确认弹窗/指令拒绝

### 下一步（Phase 2a 续，按 DEC-035 收束）
- [ ] 可控执行底座：权限作用域、执行预览、拒绝后降级、审计回放
- [ ] Web 控制面：多步任务 plan / 确认 / 执行摘要
- [ ] 高价值工具闭环：浏览器截图/填表、数据库查询、受限代码执行
- [ ] 其后再做 Skills 框架与飞书渠道
