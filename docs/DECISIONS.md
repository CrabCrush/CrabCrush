...
- 不要通读全文：**按需用 `DEC-xxx` 或关键词定位**到相关决策即可。
- 如果你只是在改一个模块的实现细节，通常不需要读与该模块无关的 DEC。
...
- **决策**：项目命名为 **CrabCrush**
- **Slogan**："做你的虾兵蟹将"
- **理由**：灵感来源 OpenClaw（龙虾🦞），CrabCrush（螃蟹🦀）延续甲壳纲主题；"Crush"暗示碾碎任务/难题/恋人，契合 AI 助手定位；"虾兵蟹将"谦逊有趣，寓意"为你服务"。
...
- **决策**：当前使用 **tsc**（零配置、足够用）；Phase 2 起如出现“需要打包/双格式/更快构建”等诉求，再评估迁移到 **tsdown**。
- **当前实现**：`tsc`
...
## DEC-034：安全可控优先 — 高风险操作必须确认

- **状态**：当前有效
- **决策**：任何可能导致（1）本地数据外泄（读文件/枚举/数据库查询结果等）、（2）系统状态变化（写/删文件、安装依赖、执行脚本/系统命令等）、（3）外部网络访问（抓取网页/搜索等）的工具调用，都必须有明确的用户确认环节：
  - **工具级**：`confirmRequired`（静态，高危工具一律确认）
  - **请求级**：`permission_request`（动态，越界访问/首次安装/按请求细分权限）
- **默认策略**：默认拒绝越界与不明确授权；只对低风险范围（如 `tools.fileBase` 内读取/列出）做无感放行
- **要求**：通道必须提供确认交互（WebChat 弹窗、钉钉等降级文本）；审计日志记录 `kind/action/allowed`；拒绝或超时需停止后续生成，避免误导
- **取舍**：相比 OpenClaw 的“直接执行”，多一步交互换取可控与安全（与 DEC-028 的数据安全边界一致）

---

## DEC-035：产品主轴收束 — 风险可控的本地执行助手优先于多渠道扩张

- **状态**：当前有效
- **背景**：CrabCrush 早期叙事强调“面向中国用户的多渠道个人 AI 助手”，这对 V1 的渠道验证是合理的。但进入 Phase 2 后，真正决定产品价值上限的，不再是多接几个聊天入口，而是能否像 Cursor / OpenClaw 一样完成多步任务，并且比它们更安全、边界更清楚。
- **问题**：
  1. 如果继续把“渠道扩张”放在主轴，团队会在飞书/企微/QQ/Telegram 等适配器上分散精力，削弱真正高价值的执行能力建设
  2. “渠道即入口”适合轻交互和通知，不适合作为高风险电脑操作的唯一主界面；权限预览、范围授权、审计回放都更适合本地 Web / Desktop 控制面
  3. 中国特色生活工具（天气、热搜、快递等）能增加功能数，但很难构成核心壁垒；“风险可控地操作本机与知识环境”才是更强的差异化
- **决策**：
  1. **产品一句话定位**：CrabCrush 是面向中国用户、可本地部署、风险可控的个人 AI 执行助手
  2. **优先级重排**：可控执行底座（权限、审计、预览、确认、降级） > 高价值工具闭环（浏览器/文件/数据库/受限执行） > 任务体验 > 新渠道扩张 > 生活工具
  3. **入口策略调整**：从“渠道即入口”调整为“渠道是入口之一”；本地 Web 控制面是执行类任务的一等入口，聊天渠道主要承担远程触发、通知回传、轻交互
  4. **路线图约束**：在可控执行底座未成熟前，不新增低壁垒功能来分散注意力，包括生活工具聚合、多 Agent、技能商店、企业版能力等
  5. **借鉴边界**：继续参考 OpenClaw 的工具与交互思路，但坚持比 OpenClaw 更严格的权限确认与审计模型，不走“默认直接执行”的路线
- **取舍**：
  - 好处：方向更聚焦，更容易形成真实壁垒，也更符合“风险可控”的长期目标
  - 代价：短期内渠道数量、功能数量增长会变慢，对外叙事不如“大而全”热闹
