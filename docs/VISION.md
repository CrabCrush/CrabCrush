...
- **什么时候读**：讨论长期方向、对外叙事、竞品取舍、路线图优先级时。
- **什么时候不读**：日常实现/修 bug/补测试时通常不需要。
- **与其他文档关系**：实现细节以 `src/` 为准；关键决策以 `docs/DECISIONS.md` 为准；当前进度以 `AGENTS.md` 为准；短期计划以 `docs/ROADMAP.md` 为准。
...
用户可以在本地 Web 控制台以及微信、钉钉、飞书等已有入口中，调度一个真正能“干活”的 AI 助手——
不需要把数据和控制权交给第三方，关键执行过程可确认、可审计、可中断。
...
CrabCrush 要解决的核心问题：**让中国用户也能拥有一个真正属于自己的、跑在自己设备上的、风险边界清楚的 AI 执行助手。**
...
|------|----------------|-----------|
| **定位** | 个人 AI 助手，面向全球（实际主要欧美） | 风险可控的个人 AI 执行助手，面向中国用户 |
| **渠道** | WhatsApp, Telegram, Slack, Discord, iMessage, Signal, Teams, Google Chat | Web 控制台 + 企业微信/钉钉/飞书/Telegram 等 |
...
- 对任务执行链路的产品化设计值得借鉴，但 CrabCrush 会把权限确认和审计做得更重
...
- 是一个**本地优先、风险可控、能直接执行任务的个人 AI 助手**
...
**基本原则：用户在哪里，助手就在哪里；但执行类任务优先在本地控制面完成。**

> 策略：本地 Web 控制面是一等入口；聊天渠道承担远程触发、通知回传、轻交互。渠道扩展不应压过执行能力建设。详见 DEC-003、DEC-035。

#### 按开发顺序

| 顺序 | 渠道 | 接入方式 | 难度 | 说明 |
|------|------|---------|------|------|
| 1st | **Web 控制台 / WebChat** | 自建 | 低 | 一等入口，承担任务确认、执行预览、审计回放 |
...
**基本原则：模型无关，用户自由选择，优先推荐国产高性价比模型；对高风险执行保持模型不可信假设。**
...
- **可控执行底座**（Phase 2）：权限请求、确认、范围授权、执行预览、审计回放
- **高价值工具**（Phase 2）：浏览器控制、文件操作、数据库查询、受限代码执行
- **渠道与扩展**（Phase 3+）：飞书/企微等渠道、知识库、语音等能力，建立在执行底座成熟之后
- **安全**（贯穿所有阶段）：内容审核、数据加密、沙箱隔离、Owner 认证（DEC-026、DEC-034、DEC-035）
...
| 工具调用 | 借鉴思路，优先级提升 | 浏览器控制和受限执行是最高价值能力（DEC-027、DEC-035） | Phase 2 核心 |
| 原生 App | 暂不做 | 先把 Web 控制面做强，避免分散精力 | 当前不做 |
...
#### V1（1.0）— 已发布；以下为当时定义的发布标准，当前进度不在本文件维护
...
- V1 范围以纯对话为硬边界；执行能力与高风险确认属于后续 Phase 2 主线

#### 远期愿景（V2+）

- [ ] 用户可在本地完成多步任务规划、确认、执行、审计回放
- [ ] 至少支持 3 类高价值执行工具（浏览器 / 文件 / 数据库或命令）
- [ ] 至少支持 3 个稳定入口（Web 控制台 + 2 个聊天渠道）
- [ ] 支持中文语音交互
- [ ] Docker 一键部署 + 宝塔面板支持
